 ...
 */
 ...
 (function(module){
  ...
  Screenshot.prototype.start = function() {
    ...
    if (!which('webkit2png')) {
      this.emit('err', 'This script requires webkit2png. http://www.paulhammond.org/webkit2png/');
      return;
    }
    ...
    function(){
      this.urls = (typeof this.url === 'string') ? [this.url] : this.url;

      this.validateGroups();

      // Process urls
      async.eachLimit(this.urls, 5, this.processUrl.bind(this), this.screenshotsComplete.bind(this));
    }.bind(this));
  };
  ...
  Screenshot.prototype.screenshotsComplete = function() {
    ...
    var date = new Date();
    var outputFile = (this.options.prefix ? this.options.prefix + '-' : '') + date.getFullYear() + (date.getMonth()+1) + date.getDate();

    fs.writeFileSync(templateFile, html, 'utf8');

    var parser = spawn('webkit2png', childArgs(templateFile, this.options.output, outputFile));
    parser.on('exit', function(code) {
      self.emit('complete', path.normalize(self.options.output + '/' + outputFile + '.png'));
    });
  };
