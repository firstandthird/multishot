 ...
 (function(module){
  ...
  Screenshot.prototype.screenshotsComplete = function() {
    glob(this.options.temp + '/*-full.png', function(err, files) {
      ...
      // Using series for easier debugging
      async.eachSeries(files, this.generateTile.bind(this), function(){
  ...
  Screenshot.prototype.generateTile = function(file, complete) {
    ...
    async.series([function(done){
      ...
      im.convert([file ,'-matte', '-bordercolor', '#ffffff', '-border', '10', file], function(err) {
        if(err) {
          console.log(err);
          self.emit('error', err);
          return;
      ...
      im.convert(['-background', 'white', '-fill', '#232323', '-font', 'Helvetica-Bold', '-pointsize', '16', 'label:' + url, textfile], function(err, meta) {
        if(err) {
          console.log(err);
          self.emit('error', err);
          return;
      ...
      im.convert(['-append', textfile, file, file], function(err) {
        if(err) {
          console.log(err);
          self.emit('error', err);
          return;
